   // Penambahan Style Kustom
    if (!document.getElementById('customEnhancementStyles')) {
        ...
        style.innerHTML = `
            ...
            .btn-salin-keren:active { transform: translateY(0) scale(0.95); }
    ...
    function initNotifUI() {
        ...
        if (actionGroup) {
            ...
            if (bells.length > 0) existingBellContainer = bells[0].parentElement;
        }
        ...
        if (existingBellContainer) {
            ...
            if (!document.getElementById('notifStyleExtras')) {
                ...
                .shake-anim i { animation: shakeBell 0.6s cubic-bezier(0.36,0.07,0.19,0.97) both; }
                .bounce-badge { animation: bounceBadge 1.2s infinite; }
                .has-unread i { color: #ef4444 !important; }
                `;
                document.head.appendChild(style);
            }
        }
    ...
    async function handleNotifClick(idBooking) {
        ...
        try { await fetchGAS({ action: "markNotifRead", idBooking: idBooking }); } catch(e) { console.error("Gagal update read status", e); }
    }

    // ================= INIT UI PENDAPATAN & MODIFIKASI LAYOUT =================
    function initPendapatanUI() {
        if (!document.getElementById('pendapatanStyles')) {
            let style = document.createElement('style');
            style.id = 'pendapatanStyles';
            style.innerHTML = `
                .info-card-item {
        ...
        // --- Desktop (Web View) Setup ---
        let globalTimer = document.getElementById('globalHeaderTimer');
        if (globalTimer && !document.getElementById('webPendapatanCard')) {
            globalTimer.style.display = 'flex';
            globalTimer.style.gap = '15px';
            globalTimer.style.alignItems = 'center';
            
            // Wrap the existing info card content (Masa Aktif/Top Up)
            let existingNodes = Array.from(globalTimer.childNodes).filter(n => n.id !== 'webPendapatanCard' && n.id !== 'webExistingCardWrap' && n.nodeType === 1);
            if (existingNodes.length > 0) {
                let wrapDesk = document.createElement('div');
                wrapDesk.className = 'info-card-item';
                wrapDesk.id = 'webExistingCardWrap';
                existingNodes.forEach(node => wrapDesk.appendChild(node));
                
                let existingBtnDesk = wrapDesk.querySelector('.btn-renew');
                let textWrapDesk = document.createElement('div');
                textWrapDesk.className = 'info-card-text';
                Array.from(wrapDesk.childNodes).forEach(node => {
                    if (node !== existingBtnDesk && node !== textWrapDesk) {
                        textWrapDesk.appendChild(node);
                    }
                });
                if(existingBtnDesk) wrapDesk.insertBefore(textWrapDesk, existingBtnDesk);
                else wrapDesk.appendChild(textWrapDesk);

                globalTimer.appendChild(wrapDesk);
            }

            // Append new Pendapatan Card
            let pendDesk = document.createElement('div');
            pendDesk.className = 'info-card-item';
            pendDesk.id = 'webPendapatanCard';
            pendDesk.innerHTML = `
            ...
            `;
            globalTimer.appendChild(pendDesk);
        }

        // --- Mobile Setup ---
        let mobileTimer = document.getElementById('mobileHeaderTimer');
        if (mobileTimer && !document.getElementById('mobPendapatanCard')) {
            // Wrap the existing info card content (Masa Aktif/Top Up)
            let existingNodes = Array.from(mobileTimer.childNodes).filter(n => n.id !== 'mobPendapatanCard' && n.id !== 'mobExistingCardWrap' && n.nodeType === 1);
            if (existingNodes.length > 0) {
                let wrapMob = document.createElement('div');
                wrapMob.className = 'mobile-info-card';
                wrapMob.id = 'mobExistingCardWrap';
                existingNodes.forEach(node => wrapMob.appendChild(node));
                
                let existingBtn = wrapMob.querySelector('.btn-renew');
                let textWrap = document.createElement('div');
                textWrap.className = 'info-card-text';
                Array.from(wrapMob.childNodes).forEach(node => {
                    if (node !== existingBtn && node !== textWrap) {
                        textWrap.appendChild(node);
                    }
                });
                if(existingBtn) wrapMob.insertBefore(textWrap, existingBtn);
                else wrapMob.appendChild(textWrap);

                mobileTimer.appendChild(wrapMob);
            }

            // Append new Pendapatan Card
            let pendMob = document.createElement('div');
            pendMob.className = 'mobile-info-card';
            pendMob.id = 'mobPendapatanCard';
            pendMob.innerHTML = `
                <div class="info-card-text">
                    <span style="font-size: 11px; font-weight: 700; color: #64748b;">Pendapatan:</span>
            ...
            `;
            mobileTimer.appendChild(pendMob);
        }
    }

    function openTarikDana() {
        showAlert("Informasi", "Fitur Tarik Dana sedang dalam pengembangan dan akan segera hadir.", "fa-info-circle");
    }
    ...
    function updateHeaderSchemeUI() {
        ...
        // Update Pendapatan UI
        let valPendapatan = formatRupiah(SESSION_COMPANY.pendapatan || 0);
        let deskText = document.getElementById('webPendapatanText');
        if (deskText) deskText.innerText = valPendapatan;
        let mobText = document.getElementById('mobPendapatanText');
        if (mobText) mobText.innerText = valPendapatan;
        ...
        realtimeSyncInterval = setInterval(async () => {
            ...
            try {
                ...
                if(res.status === "success") {
                    ...
                    if (res.pendapatan !== undefined) SESSION_COMPANY.pendapatan = res.pendapatan;
    ...
    function updateSubQty(dir) { subMultiplier += dir; if(subMultiplier < 1) subMultiplier = 1; if(subMultiplier > 12) subMultiplier = 12; calculateSubPrice(); }
            ...
            globalRekeningList.forEach(rek => {
                htmlRek += `<div class="rek-card-modern" style="position:relative;">
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px;">
                                <div><p class="norek" style="margin:0; font-size:15px; font-weight:700; color:#1E90FF;">${rek.norek}</p>
                                <p class="name" style="margin:2px 0 0 0; font-size:12px; color:#64748b;">a/n ${rek.atasNama}</p></div>
    ...
    function handleCustomNominalInput(input) {
        ...
        let rawVal = input.value.replace(/[^0-9]/g, ''); let val = parseInt(rawVal, 10) || 0;
        selectedTopUpAmount = val;
        if (val > 0) { input.value = new Intl.NumberFormat('id-ID').format(val); } else { input.value = ""; }
    ...
    async function submitRealTopUp() {
        ...
        reader.onload = async (e) => {
            ...
            } catch (err) { showLoading(false); showAlert("Error Koneksi", err.message, "fa-wifi", true); } 
            finally { btn.innerHTML = oriTxt; btn.disabled = false; }
        };
        reader.readAsDataURL(fileInput.files[0]);
    }
    ...
    function renderRiwayatCards(dataList) {
        const container = document.getElementById('riwayatCardContainer');
        if(dataList.length === 0) { container.innerHTML = `<div class="empty-state" style="text-align:center; padding: 40px 0; color:#94a3b8;"><i class="fa-solid fa-clock-rotate-left" style="font-size:40px; margin-bottom:12px;"></i><p>Belum ada riwayat transaksi.</p></div>`; return; }
        let htmlCards = '';
    ...
    function renderTopUpCards(dataList) {
        const container = document.getElementById('riwayatCardContainer');
        if(dataList.length === 0) { container.innerHTML = `<div class="empty-state" style="text-align:center; padding: 40px 0; color:#94a3b8;"><i class="fa-solid fa-clock-rotate-left" style="font-size:40px; margin-bottom:12px;"></i><p>Belum ada riwayat top up.</p></div>`; return; }
        let htmlCards = '';
        dataList.forEach((item, idx) => {
            let badgeClass = item.status.toLowerCase() === 'selesai' ? 'selesai' : (item.status.toLowerCase() === 'ditolak' ? 'ditolak' : 'pending');
            let d = new Date(item.tanggal); 
    ...
    function openRiwayatImageModal(imgUrl) {
        ...
        if (imgPreview) { imgPreview.src = directUrl; }
    ...
    function closeRiwayatImageModal(fromPopState = false) { 
        let modal = document.getElementById('riwayatSidebarModal');
        if(modal) { modal.style.right = '-500px'; }
    ...
    function openSeatCalendar() { let parsed = new Date(currentSeatDate); if(!isNaN(parsed.getTime())) { currentSeatCalYear = parsed.getFullYear(); currentSeatCalMonth = parsed.getMonth(); } renderSeatCalendar(); document.getElementById('seatCalendarModal').classList.add('open'); pushBackButtonHandler('seatCalendar'); }
   ...
   async function handleLogin() { 
    ...
    async function handleCompanyRegister() { 
        ...
        if(!selectedSkemaValue) { showAlert("Pilih Skema", "Pilih skema pembayaran terlebih dahulu.", "fa-handshake", true); return; }
        ...
        try { 
            ...
            if(res.status === "success") { 
                SESSION_COMPANY = { namaPerusahaan: n, deskripsiPerusahaan: d, logo: base64CompLogo, noHpPerusahaan: h, alamatPerusahaan: a, skemaPembayaran: selectedSkemaValue, saldo: 0, komisiRate: 0, pendapatan: 0 };
                if (res.expiredAt) {
                    SESSION_USER.expiredAt = res.expiredAt;
